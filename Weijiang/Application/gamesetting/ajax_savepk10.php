...
if($_GET['form'] == 'form1'){
    ...
    update_query("fn_lottery1", array("da" => $da, 'xiao' => $xiao, 'dan' => $dan, 'shuang' => $shuang, 'long' => $long, 'hu' => $hu, 'dadan' => $dadan, 'xiaodan' => $xiaodan, 'dashuang' => $dashuang, 'xiaoshuang' => $xiaoshuang, 'heda' => $heda, 'hexiao' => $hexiao, 'hedan' => $hedan, 'heshuang' => $heshuang, 'he341819' => $he341819, 'he561617' => $he561617, 'he781415' => $he781415, 'he9101213' => $he9101213, 'he11' => $he11, 'tema' => $tema), array('roomid' => $_SESSION['agent_room']));
    echo '<script>alert("保存成功~感谢使用!"); window.location.href="/Weijiang/index.php?m=g_setting&g=pk10";</script>';
    ...
    update_query("fn_lottery1", array("daxiao_min" => $daxiao_min, 'daxiao_max' => $daxiao_max, 'danshuang_min' => $danshuang_min, 'danshuang_max' => $danshuang_max, 'longhu_min' => $longhu_min, 'longhu_max' => $longhu_max, 'tema_min' => $tema_min, 'tema_max' => $tema_max, 'he_min' => $he_min, 'he_max' => $he_max, 'zuhe_min' => $zuhe_min, 'zuhe_max' => $zuhe_max), array('roomid' => $_SESSION['agent_room']));
    echo '<script>alert("保存成功~感谢使用!"); window.location.href="/Weijiang/index.php?m=g_setting&g=pk10";</script>';
}elseif($_GET['form'] == 'form3'){
    $open = $_POST['opengame'] == 'on' ? 'true' : 'false';
    $fengtime = $_POST['fengtime'];
    update_query("fn_lottery1", array("fengtime" => $fengtime, 'gameopen' => $open), array('roomid' => $_SESSION['agent_room']));
    echo '<script>alert("保存成功~感谢使用!"); window.location.href="/Weijiang/index.php?m=g_setting&g=pk10";</script>';
}elseif($_GET['form'] == 'form4'){
    $content = $_POST['customcont'];
    update_query("fn_lottery1", array("rules" => $content), array('roomid' => $_SESSION['agent_room']));
    echo '<script>alert("保存成功~感谢使用!"); window.location.href="/Weijiang/index.php?m=g_setting&g=pk10";</script>';
}
?>