...
switch ($type) {
    case 'up':
        $up = update_query("fn_upmark", array("status" => "已处理"), array("id" => $id['id'], 'status' => '未处理'));
        if ($up) {
            用户_上分($userid, $money, $room);
            Broadcast("@$username,您的上分请求已同意!", $game);
            echo json_encode(array("success" => true));
            exit();
        } else {
            echo json_encode(array('success' => false, 'msg' => '已同意，无需再次同意！'));
            exit();
        }
        break;
    case "down":
        if (get_query_val("fn_setting", "setting_downmark", array("roomid" => $room)) != 'true') {
            用户_下分($userid, $money, $room);
            Broadcast("@$username,您的下分请求已同意!", $game);
        }
        echo json_encode(array("success" => true));
        ...
        break;
    case "exit":
        if (get_query_val("fn_setting", "setting_downmark", array("roomid" => $room)) == 'true' && $id['type'] == '下分') {
        ...
        break;
    case "exit1":
        select_query('fn_upmark', '*', "roomid= '$room' and type='上分' and status = '未处理'");
        while ($con = db_fetch_array()) {
            $cons[] = $con;
        }
        foreach ($cons as $conv) {
            update_query("fn_upmark", array("status" => "已拒绝"), array("id" => $conv['id']));
        }
        echo json_encode(array("success" => true));
        exit();
        break;
    case "set":
        update_query("fn_setting", array("setting_downmark" => $_POST['status']), array('roomid' => $room));
...
function Broadcast($Content, $game)
{
    adminBroadcast($Content, '', '', $_SESSION['agent_room'], $game, 'S1', 'system');
}
