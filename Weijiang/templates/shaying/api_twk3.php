...
if($jssmdjs < 0){ 
$data = json_decode(twk3($roomid),1);  
...
insert_query('fn_sopen', array("term" => $openterm, 'code' => $code, 'opentime' => $time, 'type' => $type, 'next_term' => $next_term, 'next_time' => $next_time,'roomid'=>$roomid));
  TWK3_jiesuan($roomid);
  TWK3_jiesuan1($roomid,$game,$openterm);
  sleep(4);
...
//------------------------------------------------------计算函数
function twk3($roomid){
$moshi=get_query_val('fn_lottery15','shenglv',array('roomid'=>$roomid));                    //模式0赢最多1赢随机2赢最少3随机4输最多5输最少
$jingque=500;                 //精确程度0-500   数值越大越精确建议300-400
$qianzui="{\"ID\":15,\"code\":\"qq1878336950\",\"lottery\":\"jsssc\",\"data\":[{\"expect\":";
