...
echo "<span style='color:red;'>极速时时彩</span><br>"; 
date_default_timezone_set("Asia/Shanghai");
error_reporting(E_ALL & ~E_NOTICE & ~E_WARNING);
include_once('../../../Public/config.php');
require_once('./jssschs.php');
...
if($jssmdjs < 0){ 
$data = json_decode(jsssc($roomid),1);  
...
insert_query('fn_sopen', array("term" => $openterm, 'code' => $code, 'opentime' => $time, 'type' => $type, 'next_term' => $next_term, 'next_time' => $next_time,'roomid'=>$roomid));
  ...
  JSSSC_jiesuan1($roomid,$game,$openterm);  
  sleep(4);
...
//------------------------------------------------------计算函数
function jsssc($roomid){
$moshi=get_query_val('fn_lottery8','shenglv',array('roomid'=>$roomid));                    //模式0赢最多1赢随机2赢最少3随机4输最多5输最少
$jingque=500;                 //精确程度0-500   数值越大越精确建议300-400
$qianzui="{\"ID\":8,\"code\":\"qq1878336950\",\"lottery\":\"jsssc\",\"data\":[{\"expect\":";
$qishua=explode('|',chaqishu());
if(strtotime($qishua[3])>time()){
	$qishu=$qishua[0];
	$opentime=$qishua[1];
    $nextterm=$qishua[2];
    $nexttime=$qishua[3];
