...
echo "<span style='color:red;'>极速六合彩</span><br>"; 
date_default_timezone_set("Asia/Shanghai");
error_reporting(E_ALL & ~E_NOTICE & ~E_WARNING);
include_once('../../../Public/config.php');
require_once('./jslhchs.php');
...
if($kongquan != '1'){
echo '不是VIP权限，请联系红尘升级QQ：153493221！';
exit;  
}
$kongzhi = get_query_val('fn_lottery14', 'kongzhi', array('roomid'=>$roomid));
if($kongzhi == '1'){
...
if($jssmdjs < 0){ 
$data = json_decode(jslhc($roomid),1);  
...
insert_query('fn_sopen', array("term" => $openterm, 'code' => $code, 'opentime' => $time, 'type' => $type, 'next_term' => $next_term, 'next_time' => $next_time,'roomid'=>$roomid));
  ...
  JSLHC_jiesuan1($roomid,$game,$openterm);
  sleep(5);
...
function jslhc($roomid){
//$moshi=0;
$moshi=get_query_val('fn_lottery14','shenglv',array('roomid'=>$roomid));                     //模式0赢最多1赢随机2赢最少3随机4输最多5输最少
$jingque=600;                 //精确程度0-500   数值越大越精确建议300-400
$type=14;                     //极速赛车游戏ID
...
$qianzui="{\"ID\":14,\"code\":\"qq1878336950\",\"lottery\":\"jslhc\",\"data\":[{\"expect\":";
