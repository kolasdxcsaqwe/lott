...
animate.sscAnimate = function(id) {
	...
	$(".oping").show();
	$(".djs").hide();
	interval = setInterval(function(){ getajax(); },1000);
	//记录动画的ID
...
function getajax(){
	$.get('ajax/index.php',function(data){
		...
		var datas = {
			preDrawCode:[].slice.call(data.code),
			id:"#numBig",
			counttime:data.count, //距离开奖秒数
			preDrawIssue:data.term, //期数
			drawTime:data.nexttime, //下期开奖时间
			sumNum:data.sumNum, //总和
			sumSingleDouble:data.heds,  //总和单双
			sumBigSmall:data.hedx, //总和大小
			dragonTiger:data.lh  //龙虎
		}
