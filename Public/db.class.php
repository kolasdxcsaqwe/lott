...
class db{
    ...
    public function __construct($config=array()){
        ...
        $this->config['DB_HOST'] = isset($config['DB_HOST'])?$config['DB_HOST']:'localhost';//数据库主机
        $this->config['DB_USER'] = isset($config['DB_USER'])?$config['DB_USER']:'wdfgt_yodo8_cn';//数据库用户名
        $this->config['DB_PWD'] = isset($config['DB_PWD'])?$config['DB_PWD']:'wdfgt_yodo8_cn';//数据库密码
        $this->config['DB_PORT'] = isset($config['DB_PORT'])?$config['DB_PORT']:3306;//数据库端口，mysql默认是3306，一般不需要修改
        $this->config['DB_NAME'] = isset($config['DB_NAME'])?$config['DB_NAME']:'wdfgt_yodo8_cn';//数据库名
