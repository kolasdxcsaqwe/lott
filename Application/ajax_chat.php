    ...
    case "send":
        ...
        if ($type == 'U3') {
            ...
            if ($content == "取消") {
                ...
                roomBroadcast($headimg,$nickname,$content,'','',$_SESSION['roomid'],$_COOKIE['game'],$type,$_SESSION['userid']);
                break;
            }
        }

        if ($type == 'U3' && $fenshuchange == false && ($BetGame == 'ny28' || $BetGame == 'xy28' || $BetGame == 'jnd28')) {
            $co = addPCBet($_SESSION['userid'], $_SESSION['username'], $_SESSION['headimg'], $content, $BetTerm, $fengpan,$BetGame);
            ...
            if (time() - $myy_time < 10) {
                Broadcast("@" . $_SESSION['username'] . " ,[$BetTerm]期还未开始投注！");
                exit;
            } else {
                $co = addBJLBet($_SESSION['userid'], $_SESSION['username'], $_SESSION['headimg'], $content, $BetTerm, $fengpan,$BetGame);
            }

        } elseif ($type == 'U3' && $fenshuchange == false && ($BetGame == 'lhc' || $BetGame == 'jslhc')) {
            $co = addLHCBet($_SESSION['userid'], $_SESSION['username'], $_SESSION['headimg'], $content, $BetTerm, $fengpan,$BetGame);
        } elseif ($type == 'U3' && $fenshuchange == false && ($BetGame == 'cqssc' || $BetGame == 'jsssc' || $BetGame == 'txffc' || $BetGame == 'azxy5')) {
            $co = addSSCBet($_SESSION['userid'], $_SESSION['username'], $_SESSION['headimg'], $content, $BetTerm, $fengpan,$BetGame);
        } elseif ($type == 'U3' && $fenshuchange == false && ($BetGame == 'kuai3' || $BetGame == 'twk3')) {
            $co = addK3Bet($_SESSION['userid'], $_SESSION['username'], $_SESSION['headimg'], $content, $BetTerm, $fengpan,$BetGame);
        } elseif ($type == 'U3' && $fenshuchange == false && ($BetGame == 'gd11x5')) {
            $co = add11X5Bet($_SESSION['userid'], $_SESSION['username'], $_SESSION['headimg'], $content, $BetTerm, $fengpan,$BetGame);
        } elseif ($type == 'U3' && $fenshuchange == false) {
            $co = addBet($_SESSION['userid'], $_SESSION['username'], $_SESSION['headimg'], $content, $BetTerm, $fengpan,$BetGame);
        }
        if (get_query_val("fn_setting", "setting_ischat", array("roomid" => $_SESSION['roomid'])) == 'open' && !$co[0] && !$fenshuchange) {
            ...
            roomBroadcast($headimg,$nickname,$content,'','',$_SESSION['roomid'],$_COOKIE['game'],$type,$_SESSION['userid'],$co[1]);
        }
        break;
}
...
{
    robotBroadcast($Content,'','',$_SESSION['roomid'],$_COOKIE['game'],'S3','system');
}
...
function CancelBet($userid, $term, $game, $fengpan)
{
    ...
    if ($chedan) {
        ...
        if ($fengpan) {
            Broadcast("@" . $_SESSION['username'] . " ,[$term]期已经停止投注！无法取消！");
            return false;
        }
        switch ($game) {
            case 'pk10':
        ...
        用户_上分($userid, $all);
        Broadcast("@{$_SESSION['username']} ,[$term]期投注已经退还!");
    }
}

function addLHCBet($userid, $nickname, $headimg, $content, $addQihao, $fengpan,$BetGame)
{
    if ($fengpan) {
    ...
    foreach ($A as $ai) {
        ...
        if ($zym_6 == '红波' || $zym_6 == '蓝波' || $zym_6 == '绿波') {
            ...
            foreach ($zym_10_分割 as $ii) {
                ...
                用户_下分($userid, $zym_5);
                if (!$_SESSION['try']) {
                    insert_query($ordertable, array('gamtype'=>$gameType,'gamename'=>$BetGame,'term' => $addQihao, 'userid' => $userid, 'username' => $nickname, 'headimg' => $headimg, 'mingci' => $ii, 'content' => $zym_6, 'money' => $zym_5, 'addtime' => 'now()', 'roomid' => $_SESSION['roomid'], 'status' => '未结算', 'jia' => $zym_8), $chat_id);
                }
            }
        }
        ...
        if (empty($zym_10)) {
            ...
            if (strpos($zym_6, '鼠') !== false || strpos($zym_6, '牛') !== false || strpos($zym_6, '虎') !== false || strpos($zym_6, '兔') !== false || strpos($zym_6, '龙') !== false || strpos($zym_6, '蛇') !== false || strpos($zym_6, '马') !== false || strpos($zym_6, '羊') !== false || strpos($zym_6, '猴') !== false || strpos($zym_6, '鸡') !== false || strpos($zym_6, '狗') !== false || strpos($zym_6, '猪') !== false) {
                ...
                foreach ($zym_6_分割 as $ii) {
                    ...
                    用户_下分($userid, $zym_5);
                    if (!$_SESSION['try']) {
                        insert_query($ordertable, array('gamtype'=>$gameType,'gamename'=>$BetGame,'term' => $addQihao, 'userid' => $userid, 'username' => $nickname, 'headimg' => $headimg, 'mingci' => '', 'content' => $ii, 'money' => $zym_5, 'addtime' => 'now()', 'roomid' => $_SESSION['roomid'], 'status' => '未结算', 'jia' => $zym_8), $chat_id);
                    }
                }
            }
        ...
        if (!empty($zym_10)) {
            ...
            if ($zym_10 == '2肖') {
                ...
                foreach ($zym_6_分割 as $v) {
                    ...
                    用户_下分($userid, $zym_5);
                    if (!$_SESSION['try']) {
                        insert_query($ordertable, array('gamtype'=>$gameType,'gamename'=>$BetGame,'term' => $addQihao, 'userid' => $userid, 'username' => $nickname, 'headimg' => $headimg, 'mingci' => $zym_10, 'content' => $v, 'money' => $zym_5, 'addtime' => 'now()', 'roomid' => $_SESSION['roomid'], 'status' => '未结算', 'jia' => $zym_8), $chat_id);
                    }
                }
            }
            ...
            if ((strpos($zym_6, '鼠') !== false || strpos($zym_6, '牛') !== false || strpos($zym_6, '虎') !== false || strpos($zym_6, '兔') !== false || strpos($zym_6, '龙') !== false || strpos($zym_6, '蛇') !== false || strpos($zym_6, '马') !== false || strpos($zym_6, '羊') !== false || strpos($zym_6, '猴') !== false || strpos($zym_6, '鸡') !== false || strpos($zym_6, '狗') !== false || strpos($zym_6, '猪') !== false) && ($zym_10 != '2肖' && $zym_10 != '3肖' && $zym_10 != '4肖' && $zym_10 != '5肖')) {
                ...
                foreach ($zym_10_分割 as $ii) {
                    ...
                    foreach ($zym_6_分割 as $iii) {
                        ...
                        用户_下分($userid, $zym_5);
                        if (!$_SESSION['try']) {
                            insert_query($ordertable, array('gamtype'=>$gameType,'gamename'=>$BetGame,'term' => $addQihao, 'userid' => $userid, 'username' => $nickname, 'headimg' => $headimg, 'mingci' => $ii, 'content' => $iii, 'money' => $zym_5, 'addtime' => 'now()', 'roomid' => $_SESSION['roomid'], 'status' => '未结算', 'jia' => $zym_8), $chat_id);
                        }
                    }
                }
            }
...
function addK3Bet($userid, $nickname, $headimg, $content, $addQihao, $fengpan,$BetGame)
{
    ...
    foreach ($A as $i) {
        ...
        if ($neirong == '大' || $neirong == '小' || $neirong == '单' || $neirong == '双' || $neirong == '大单' || $neirong == '小单' || $neirong == '大双' || $neirong == '小双' || $neirong == '顺子' || $neirong == '豹子' || $neirong == '对子' || $neirong == '三杂' || $neirong == '二杂' || $neirong == '123' || $neirong == '234' || $neirong == '345' || $neirong == '456' || $neirong == '111' || $neirong == '222' || $neirong == '333' || $neirong == '444' || $neirong == '555' || $neirong == '666' || $neirong == '1' || $neirong == '2' || $neirong == '3' || $neirong == '4' || $neirong == '5' || $neirong == '6' || $neirong == '11' || $neirong == '22' || $neirong == '33' || $neirong == '44' || $neirong == '55' || $neirong == '66' || $neirong == '123' || $neirong == '124' || $neirong == '125' || $neirong == '126' || $neirong == '134' || $neirong == '135' || $neirong == '136' || $neirong == '145' || $neirong == '146' || $neirong == '156' || $neirong == '234' || $neirong == '235' || $neirong == '236' || $neirong == '245' || $neirong == '246' || $neirong == '256' || $neirong == '345' || $neirong == '346' || $neirong == '356' || $neirong == '456' || $neirong == '12' || $neirong == '13' || $neirong == '14' || $neirong == '15' || $neirong == '16' || $neirong == '23' || $neirong == '24' || $neirong == '25' || $neirong == '26' || $neirong == '34' || $neirong == '35' || $neirong == '36' || $neirong == '45' || $neirong == '46' || $neirong == '56') {
            ...
            $touzhu = true;
            用户_下分($userid, $jine);
            insert_query($ordertable, array('gamtype'=>$gameType,'gamename'=>$BetGame,"term" => $addQihao, 'userid' => $userid, 'username' => $_SESSION['username'], 'headimg' => $_SESSION['headimg'], 'mingci' => $mingci, 'content' => $neirong, 'money' => $jine, 'addtime' => 'now()', 'roomid' => $_SESSION['roomid'], 'status' => '未结算', 'jia' => $jia));
            continue;
        }
    }
    if ($zym_2 != "") {
...
function addBJLBet($userid, $nickname, $headimg, $content, $addQihao, $fengpan,$BetGame)
{
...
function addBet($userid, $nickname, $headimg, $content, $addQihao, $fengpan,$BetGame)
{
    if ($fengpan) {
        Broadcast("@" . $nickname . " ,[$addQihao]期已经停止投注！下注无效！");
        $carr[0] = false;
    ...
    foreach ($A as $ai) {
        ...
        if ($zym_10 == '和') {
            ...
            if ($zym_6 == "大" || $zym_6 == "小" || $zym_6 == "单" || $zym_6 == "双") {
                ...
                用户_下分($userid, $zym_5);
                insert_query($ordertable, array('gamtype'=>$gameType,'gamename'=>$BetGame,'term' => $addQihao, 'userid' => $userid, 'username' => $nickname, 'headimg' => $headimg, 'mingci' => $zym_10, 'content' => $zym_6, 'money' => $zym_5, 'addtime' => 'now()', 'roomid' => $_SESSION['roomid'], 'status' => '未结算', 'jia' => $zym_8), $chat_id);
                $touzhu = true;
                continue;
            }
            $zym_6_分割 = 和值分割($zym_6);
            foreach ($zym_6_分割 as $ii) {
                ...
                $touzhu = true;
                用户_下分($userid, $zym_5);
                insert_query($ordertable, array('gamtype'=>$gameType,'gamename'=>$BetGame,'term' => $addQihao, 'userid' => $userid, 'username' => $nickname, 'headimg' => $headimg, 'mingci' => $zym_10, 'content' => $ii, 'money' => $zym_5, 'addtime' => 'now()', 'roomid' => $_SESSION['roomid'], 'status' => '未结算', 'jia' => $zym_8), $chat_id);
                continue;
            }
            continue;
        }
...
function addPCBet($userid, $nickname, $headimg, $content, $addQihao, $fengpan,$BetGame)
{
    if ($fengpan) {
        Broadcast("@" . $nickname . " ,[$addQihao]期已经停止投注！下注无效！");
        $carr[0] = false;
    ...
    foreach ($A as $i) {
        ...
        if ($zym_6 == '大' || $zym_6 == '小' || $zym_6 == '单' || $zym_6 == '双') {
            ...
            if ($zym_4 == 'true') {
                switch ($zym_6) {
                    case '大':
                        $betting1 = get_query_val('fn_pcorder', 'content', array('userid' => $userid, 'term' => $addQihao, 'roomid' => $_SESSION['roomid'], 'status' => '未结算', 'content' => '小单'));
                        $betting2 = get_query_val('fn_pcorder', 'content', array('userid' => $userid, 'term' => $addQihao, 'roomid' => $_SESSION['roomid'], 'status' => '未结算', 'content' => '小双'));
                        break;
                    case "小":
                        $betting1 = get_query_val('fn_pcorder', 'content', array('userid' => $userid, 'term' => $addQihao, 'roomid' => $_SESSION['roomid'], 'status' => '未结算', 'content' => '大单'));
                        $betting2 = get_query_val('fn_pcorder', 'content', array('userid' => $userid, 'term' => $addQihao, 'roomid' => $_SESSION['roomid'], 'status' => '未结算', 'content' => '大双'));
                        break;
                    case "单":
                        $betting1 = get_query_val('fn_pcorder', 'content', array('userid' => $userid, 'term' => $addQihao, 'roomid' => $_SESSION['roomid'], 'status' => '未结算', 'content' => '小双'));
                        $betting2 = get_query_val('fn_pcorder', 'content', array('userid' => $userid, 'term' => $addQihao, 'roomid' => $_SESSION['roomid'], 'status' => '未结算', 'content' => '大双'));
                        break;
                    case "双":
                        $betting1 = get_query_val('fn_pcorder', 'content', array('userid' => $userid, 'term' => $addQihao, 'roomid' => $_SESSION['roomid'], 'status' => '未结算', 'content' => '小单'));
                ...
                if ($betting1 != "" || $betting2 != "") {
                    if ($zym_14_例外 > 0 && $zym_7 + $zym_5 > $zym_14_例外) {
                        $touzhu = true;
                        用户_下分($userid, $zym_5);
                        insert_query("fn_pcorder", array('gamtype'=>$gameType,'gamename'=>$BetGame,"term" => $addQihao, 'userid' => $userid, 'username' => $_SESSION['username'], 'headimg' => $_SESSION['headimg'], 'content' => $zym_6, 'money' => $zym_5, 'addtime' => 'now()', 'roomid' => $_SESSION['roomid'], 'status' => '未结算', 'jia' => $zym_8), $chat_id);
                    } else {
                        $jinzhi = true;
                        $zym_2 = $zym_6 . '/' . $zym_5;
                        continue;
            ...
            if ($zym_4 == 'true') {
                switch ($zym_6) {
                    case '大单':
                        $betting1 = get_query_val('fn_pcorder', 'content', array('userid' => $userid, 'term' => $addQihao, 'roomid' => $_SESSION['roomid'], 'status' => '未结算', 'content' => '双'));
                        $betting2 = get_query_val('fn_pcorder', 'content', array('userid' => $userid, 'term' => $addQihao, 'roomid' => $_SESSION['roomid'], 'status' => '未结算', 'content' => '小'));
                        break;
                    case "小单":
                        $betting1 = get_query_val('fn_pcorder', 'content', array('userid' => $userid, 'term' => $addQihao, 'roomid' => $_SESSION['roomid'], 'status' => '未结算', 'content' => '大'));
                        $betting2 = get_query_val('fn_pcorder', 'content', array('userid' => $userid, 'term' => $addQihao, 'roomid' => $_SESSION['roomid'], 'status' => '未结算', 'content' => '双'));
                        break;
                    case "大双":
                        $betting1 = get_query_val('fn_pcorder', 'content', array('userid' => $userid, 'term' => $addQihao, 'roomid' => $_SESSION['roomid'], 'status' => '未结算', 'content' => '小'));
...
function add11X5Bet($userid, $nickname, $headimg, $content, $addQihao, $fengpan,$BetGame)
{
    if ($fengpan) {
        Broadcast("@" . $nickname . " ,[$addQihao]期已经停止投注！下注无效！");
        $carr[0] = false;
    ...
    foreach ($A as $ai) {
        ...
        if ($zym_10 == '总') {
            if ($zym_6 == '大' || $zym_6 == '小') {
                ...
                用户_下分($userid, $zym_5);
                insert_query($ordertable, array('gamtype'=>$gameType,'gamename'=>$BetGame,'term' => $addQihao, 'userid' => $userid, 'username' => $nickname, 'headimg' => $headimg, 'mingci' => $zym_10, 'content' => $zym_6, 'money' => $zym_5, 'addtime' => 'now()', 'roomid' => $_SESSION['roomid'], 'status' => '未结算', 'jia' => $zym_8, 'game' => $_COOKIE['game']), $chat_id);
                $touzhu = true;
                continue;
        ...
        if ($zym_6 == "大" || $zym_6 == "小" || $zym_6 == "单" || $zym_6 == "双") {
            ...
            foreach ($zym_10_分割 as $ii) {
                ...
                $touzhu = true;
                用户_下分($userid, $zym_5);
                insert_query($ordertable, array('gamtype'=>$gameType,'gamename'=>$BetGame,'term' => $addQihao, 'userid' => $userid, 'username' => $nickname, 'headimg' => $headimg, 'mingci' => $ii, 'content' => $zym_6, 'money' => $zym_5, 'addtime' => 'now()', 'roomid' => $_SESSION['roomid'], 'status' => '未结算', 'jia' => $zym_8, 'game' => $_COOKIE['game']), $chat_id);
                continue;
            }
            continue;
        }
        if ($zym_10 == '前三' || $zym_10 == '中三' || $zym_10 == '后三' || preg_match("/(前三|中三|后三)/u", $zym_10)) {
            ...
            foreach ($arr as $i) {
                ...
                $touzhu = true;
                用户_下分($userid, $zym_5);
                insert_query($ordertable, array('gamtype'=>$gameType,'gamename'=>$BetGame,'term' => $addQihao, 'userid' => $userid, 'username' => $nickname, 'headimg' => $headimg, 'mingci' => $i, 'content' => $zym_6, 'money' => $zym_5, 'addtime' => 'now()', 'roomid' => $_SESSION['roomid'], 'status' => '未结算', 'jia' => $zym_8, 'game' => $_COOKIE['game']), $chat_id);
                continue;
            }
            continue;
        }
        ...
        foreach ($zym_10_分割 as $ii) {
            ...
            foreach ($zym_6_分割 as $iii) {
                ...
                $touzhu = true;
                用户_下分($userid, $zym_5);
                insert_query($ordertable, array('gamtype'=>$gameType,'gamename'=>$BetGame,'term' => $addQihao, 'userid' => $userid, 'username' => $nickname, 'headimg' => $headimg, 'mingci' => $ii, 'content' => $iii, 'money' => $zym_5, 'addtime' => 'now()', 'roomid' => $_SESSION['roomid'], 'status' => '未结算', 'jia' => $zym_8, 'game' => $_COOKIE['game']), $chat_id);
            }
        }
        continue;
    }
...
function addSSCBet($userid, $nickname, $headimg, $content, $addQihao, $fengpan,$BetGame)
{
    if ($fengpan) {
        Broadcast("@" . $nickname . " ,[$addQihao]期已经停止投注！下注无效！");
        $carr[0] = false;
    ...
    foreach ($A as $ai) {
        ...
        if ($zym_10 == '前三' || $zym_10 == '中三' || $zym_10 == '后三' || preg_match("/(前三|中三|后三)/u", $zym_10)) {
            ...
            foreach ($arr as $i) {
                ...
                $touzhu = true;
                用户_下分($userid, $zym_5);
                insert_query($ordertable, array('gamtype'=>$gameType,'gamename'=>$BetGame,'term' => $addQihao, 'userid' => $userid, 'username' => $nickname, 'headimg' => $headimg, 'mingci' => $i, 'content' => $zym_6, 'money' => $zym_5, 'addtime' => 'now()', 'roomid' => $_SESSION['roomid'], 'status' => '未结算', 'jia' => $zym_8), $chat_id);
                continue;
            }
            continue;
        }
