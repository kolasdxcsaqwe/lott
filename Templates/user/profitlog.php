...
$info = getextend($_SESSION['userid'],$time);
?>
<!doctype html>
<html lang="en">
    ...
    <div class="wx_cfb_container wx_cfb_account_center_container">
        <div class="wx_cfb_account_center_wrap">
            ...
            <div class="wx_cfb_entry_list">
                <br>
                <div class="kv_tb_list clearfix">
                    <button class="btn btn-primary kv_item" style="text-align: center;border-radius: 35px;" onclick="window.location.href='profitlog.php?time=1'">今日</button>
                    <button class="btn btn-primary kv_item" style="text-align: center;border-radius: 35px;" onclick="window.location.href='profitlog.php?time=7'">昨日</button>
                    <button class="btn btn-primary kv_item" style="text-align: center;border-radius: 35px;" onclick="window.location.href='profitlog.php?time=30'">30日</button>
                </div>
                <br>
                <table class="table table-bordered table-striped">
                    <thead>
                        <tr>
                            ...
                            <th style="text-align:center">总中奖</th>
                            <th style="text-align:center">总盈亏</th>
                        </tr>
                    </thead>
                    <tbody>
                    <?php select_query("fn_marklog", '*', "`userid` = '{$_SESSION['userid']}' and `roomid` = '{$_SESSION['roomid']}' and `addtime` between '$time 00:00:00' and '$time2 23:59:59'");
                    while($con = db_fetch_array()){
                    $cons[] = $con;
                    ?>
                        <tr align="center">
                            <td><?php echo $con['addtime']?></td>         
                            <td><?php echo $info['liu']?></td>
                            <td><?php echo $info['money']?></td>
                            <td><?php echo $info['yk']?></td>
                        </tr>
                    <?php }
                     if(count($cons) == 0){
                       echo '<tr><td colspan=4 style="text-align: center">您没有任何投注记录</td></tr>';
                     }?>
                    </tbody>
                </table>
            </div>
